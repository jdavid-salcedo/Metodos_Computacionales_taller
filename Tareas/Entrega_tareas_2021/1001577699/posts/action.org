...
#+SETUPFILE: ~/Personal/github_repos/Metodos_Computacionales_taller/Tareas/Entrega_tareas_2021/1001577699/org-html-themes/org/theme-readtheorg-local.setup

#+begin_src jupyter-python :session python :async yes :results raw drawer
...
def path(N, theta_0=0, theta_f=th_f):
    thetas = fmin_powell(action, x0=np.zeros(int(N)), disp=False)
    # fix the boundary
    thetas[0], thetas[-1] = theta_0, theta_f
    return thetas
...
#+RESULTS:
:results:
: array([0.        , 0.00559656, 0.01118721, 0.01676633, 0.02232795,
:        0.02786655, 0.03337641, 0.03885188, 0.04428761, 0.0496777 ,
:        0.05501686, 0.06029932, 0.06551943, 0.07067203, 0.07575125,
:        0.08075244, 0.08567028, 0.09049991, 0.09523704, 0.09987613,
...
#+RESULTS:
:results:
[[file:./.ob-jupyter/2f8538e051c53a643ea5bc02c96bb050dd06d5fa.svg]]
:end:
#+begin_src jupyter-python :session python :async yes :results raw drawer
...
#+RESULTS:
:results:
[[file:./.ob-jupyter/699e44b7766c82db391f230251246c381986507d.svg]]
:end:
...
def _path(N, x_0=0.707106, x_f=0.707106):
    x = fmin_powell(_action, x0=np.zeros(int(N)), disp=False)
    # fix the boundary
    x[0], x[-1] = x_0, x_f
    return x
...
:results:
#+begin_example
array([0.707106  , 0.72896325, 0.74992769, 0.77043329, 0.79009548,
       0.80897772, 0.82703792, 0.84429362, 0.86070009, 0.87626238,
       0.89095822, 0.9047749 , 0.91770245, 0.92972336, 0.9408303 ,
       0.95100818, 0.96024931, 0.9685426 , 0.97588023, 0.98225472,
       0.98766076, 0.99209205, 0.99554648, 0.99801834, 0.99950339,
       1.00000197, 0.99951   , 0.99803155, 0.99557784, 0.99214157,
       0.98772743, 0.98233857, 0.9759515 , 0.96860116, 0.96031583,
       0.95108267, 0.9409571 , 0.92990285, 0.9178419 , 0.90487507,
       0.89099313, 0.87623182, 0.86077174, 0.84446212, 0.82722865,
       0.80917874, 0.79019668, 0.77043472, 0.75007153, 0.72894848,
       0.707106  ])
#+end_example
:end:
...
#+RESULTS:
:results:
[[file:./.ob-jupyter/9e9b3a9fe3379ed9cb48981144eecc347cefa22e.svg]]
:end:
...
#+RESULTS:
:results:
[[file:./.ob-jupyter/c92448f663537d3e4b63364fdedaf5512265519f.svg]]
:end:
...
#+RESULTS:
:results:
[[file:./.ob-jupyter/80ac518bbdd6e186cdc0f27e7e56b728efbeda2c.svg]]
:end:
...
class Lagrangian:
    ...
    def path(self, N, args=()):
        ...
        # minimize the action
        path = fmin_powell(self.action, x0=np.zeros(int(N)), args=args, disp=False)
...
#+RESULTS:
:results:
[[file:./.ob-jupyter/0c1d0b7970981cc85f90508430a6315a44eb15aa.svg]]
:end:

As before, we can build up the energy plot, so as to prove that it is conserved:
#+begin_src jupyter-python :session python :async yes :results raw drawer
def energy(m=0.2, L=1, g=9.8) :
    # get the graph of both the expected function and the minimised one
...
#+RESULTS:
:results:
[[file:./.ob-jupyter/9f8a569ccce6c154da0ccbbc55105e5c2c35be04.svg]]
:end:
...
#+RESULTS:
:results:
[[file:./.ob-jupyter/3a6e1c3d6ff5f04ad5bcd141fdb7734c417b0a34.svg]]
:end:
